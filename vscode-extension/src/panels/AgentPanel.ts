...
export class AgentPanel implements vscode.WebviewViewProvider {
  public static readonly viewType = 'agentic-dev-tool.agentPanel';

  // Model used for Ollama requests — adjust to whatever you have pulled locally
  private readonly _model = 'qwen2.5-coder:3b';
    ...
    webviewView.webview.onDidReceiveMessage(async (data) => {
      if (data.type === 'sendMessage') {
        try {
          ...
          const text: string = json.response ?? 'No response from model.';
  ...
  private _getHtml(model: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
